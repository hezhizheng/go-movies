	...
	c.OnHTML("ul#sddm li", func(e *colly.HTMLElement) {
		...
		e.ForEach("div a", func(i int, element *colly.HTMLElement) {
			...
			if subCategoryName != categoryName {
				...
				if !InArray(subCategoryName,[]string{"福利片"}) {
					// 追加
					Smutex.Lock()
		...
		if !InArray(categoryName,[]string{"","福利片","综艺","解说"}) {
			// 追加
			Smutex.Lock()
...
func SpiderOKMovies(cateUrl string) {
	...
	c.OnHTML(".pages button", func(e *colly.HTMLElement) {
...
func MoviesInfo(url string) MoviesDetail {
	...
	c.OnScraped(func(_ *colly.Response) {
		...
		if md.Name != "" {
			Smutex.Lock()
			t := RedisDB.HMSet(moviesDetail+Index2vod(url)+":movie_name:"+md.Name, _moviesInfo).Err()
			log.Println(t)
			Smutex.Unlock()
		}
...
func CategoryToPageUrl(categoryUrl string, page string) string {
	pageUrl := strings.Replace(categoryUrl, ".html", "/page/" + page + ".html", -1)
	return pageUrl
}
